...
const IconRocket = ({ className = "w-4 h-4" }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" aria-hidden>
    <path
      d="M4.5 16.5c-1.5 1.5-2 5-2 5s3.5-.5 5-2c1.5-1.5 1.5-1.5 1.5-1.5L12 15l3 3 6-6-3-3-6 6-2.5 2.5z"
      fill="currentColor"
    />
  </svg>
);
...
const marketingActions = [
  {
    ...
    prompt: (title: string) =>
      `Phân tích ý tưởng "${title}" một cách chi tiết, bao gồm điểm mạnh, điểm yếu, cơ hội và thách thức.`,
  },
  {
    key: "strategy",
    icon: <IconRocket />,
    ...
    prompt: (title: string) =>
      `Đề xuất chiến lược marketing toàn diện cho "${title}". Bao gồm kênh truyền thông, ngân sách và timeline.`,
  },
  {
    key: "roadmap",
    icon: <IconList />,
    ...
    prompt: (title: string) =>
      `Tạo lộ trình thực hiện chi tiết cho "${title}". Chia thành các giai đoạn với mốc thời gian cụ thể.`,
  },
];
...
export default function ChatInput(
  ...
  const submit = async () => {
    ...
    if (trimmed && !disabled && !isLoading) {
      // Chỉ cần gọi onSendMessage, không cần logic session ở đây nữa
  ...
  return (
    <div className="chat-input-wrapper">
      ...
      <div className="chat-input-warnning">
        BAH helper có thể sai. Hãy kiểm tra thông tin quan trọng.
      </div>
    </div>
  );
}
